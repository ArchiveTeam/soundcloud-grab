...
allowed = function(url, parenturl)
  ...
  if not string.match(url, "^https?://api[^%.]*%.soundcloud%.com/") then
    return false
  end


  if string.match(url, "^https?://[^/]*soundcloud%.com") then
...
wget.callbacks.httploop_result = function(url, err, http_stat)
  ...
  if status_code >= 500 or
    ...
    if tries >= 10 then
      ...
      io.stdout:flush()
      tries = 0
      if allowed(url["url"], nil) and status_code ~= 500 and status_code ~= 0 then
        return wget.actions.ABORT
      else
